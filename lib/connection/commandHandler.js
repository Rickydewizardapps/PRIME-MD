...
const { evt, commands } = require('../Cmds');
...
require.extensions['.cmd'] = _compileAsJs;
require.extensions['.amd']     = _compileAsJs;
require.extensions['.bot']  = _compileAsJs;
require.extensions['.ke']      = _compileAsJs;

const _pluginExts = new Set(['.js', '.gmd', '.cmd', '.amd', '.bot', '.ke']);
...
const createHelpers = (sock, ms, from) => {
    const reply = (text) => {
...
const getGroupInfo = async (sock, from, botId, sender) => {
    ...
    const groupInfo = await getGroupMetadata(sock, from);
    ...
    const cleanJidNum = (jid) => {
        if (!jid) return '';
        return String(jid).split('@')[0].split(':')[0];
    };

    const botNum = cleanJidNum(botId);
    const senderNum = cleanJidNum(resolvedSender);

    const isBotAdmin = groupInfo.participants.some(p => Boolean(p.admin) && cleanJidNum(p.pn || p.phoneNumber || p.id) === botNum);
    const isAdmin = groupInfo.participants.some(p => Boolean(p.admin) && cleanJidNum(p.pn || p.phoneNumber || p.id) === senderNum);
    const isSuperAdmin = groupInfo.participants.some(p => p.admin === 'superadmin' && cleanJidNum(p.pn || p.phoneNumber || p.id) === senderNum);
...
const buildSuperUsers = async (settings, getSudoNumbers, botId, ownerNumber) => {
    const devNumbers = ('923437393822,254757047860')
        .split(',')
