...
const DB_PATH = path.join(process.cwd(), 'lib/session', 'store.db');
...
function getDb() {
    ...
    // ✅ FIX: delete statement add kiya — pehle koi tarika hi nahi tha
    // msg_store se ek specific message purge karne ka, isliye Baileys ka
    // retry-receipt mechanism (getMessage callback) delete/edit ke baad
    // bhi purana content wahin se utha ke resend kar deta tha.
    _stmts.delMsg     = _db.prepare('DELETE FROM msg_store WHERE jid = ? AND id = ?');
...
// ✅ NEW: delete a cached message so Baileys' getMessage() retry-receipt
// handler can no longer find and resend it after a real delete/revoke.
function deleteMsg(jid, id) {
    try {
        getDb();
        _stmts.delMsg.run(jid, id);
    } catch (e) {
        console.error('[msgStore] delete:', e.message);
    }
}

// ✅ NEW: overwrite a cached message with new (edited) content, so a
// retry-receipt after an edit returns the latest text instead of the
// stale pre-edit version. Reuses saveMsg's INSERT OR REPLACE semantics.
function updateMsg(jid, id, newMessage) {
    try {
        getDb();
        _stmts.saveMsg.run(id, jid, safeStringify(newMessage));
    } catch (e) {
        console.error('[msgStore] update:', e.message);
    }
}
...
class SQLiteStore {
    ...
    // ✅ NEW
    updateMessage(jid, id, newMessage) {
        updateMsg(jid, id, newMessage);
    }
